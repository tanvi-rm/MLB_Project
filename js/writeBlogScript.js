...
document.querySelectorAll(".language-btn").forEach((btn) => {
  btn.addEventListener("click", function () {
    document
      ...
      .forEach((b) => b.classList.replace("bg-purple-600", "bg-gray-200"));
...
function validateFields() {
  ...
  if (!description) {
    ...
    valid = false;
  } else hideError("error-description");
  if (!tags) {
    showError("error-tags", "Tags are required.");
    valid = false;
  } else hideError("error-tags");
  if (!content || content === "<p><br></p>") {
    showError("error-content", "Content is empty.");
    valid = false;
  } else hideError("error-content");
  if (!selectedLanguage) {
    showError("error-language", "Select a language.");
    valid = false;
  } else hideError("error-language");
  if (!coverImage) {
    showError("error-image", "Upload cover image.");
    valid = false;
  } else hideError("error-image");
...
document
  ...
  .addEventListener("click", async function () {
    ...
    formData.append(
      "category",
      document.getElementById("blog-category").value.trim()
    );
    formData.append(
      "description",
      document.getElementById("blog-description").value.trim()
    );
    ...
    try {
      const res = await fetch("/api/write-blog/write", {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
